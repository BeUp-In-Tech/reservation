...
class ChatService:
    ...
    async def send_message(
        ...
        print("DEBUG selected_service_id:", result_state.get("selected_service_id"))
        print("DEBUG parsed_intent:", result_state.get("parsed_intent"))
        ...
        if intent == "cancel_booking":
            ...
            if tracking_id:
                try:
                    ...
                    if booking_info:
                        result_state["response"] = f"I found your booking **{tracking_id}** for {booking_info.get('service_name', 'your service')}. Are you sure you want to cancel it? This cannot be undone."
                        result_state["mentioned_booking_id"] = tracking_id
                    else:
                        result_state["response"] = f"I couldn't find a booking with ID **{tracking_id}**. Please check the booking ID and try again. It should look like BK-XXXXXX."
                except Exception as e:
                    result_state["response"] = f"I couldn't find that booking. Please provide a valid booking ID like BK-XXXXXX."
            else:
                result_state["response"] = "Please provide your booking ID (like BK-XXXXXX) so I can cancel it for you."
